...
function forwardWebsocketData(message) {
	...
	debug('got message %o for uri %o', payload.name, payload.data && payload.data.uri);
	...
	// XXX I’m not proud of this solution, but currently there’s no way in 
	// `node-websocket` package to accept websocket connection and send custom 
	// HTTP header. To restrict transmitted LiveStyle messages, I’m sending
	// list of allowed origins as `Origin` HTTP header. The first origin is 
	// always is a publicId. In order to properly apply updates in RV clients,
	// given message URI must be rewritten with this publicId
	messageUri.host = this.remoteClient.origin[0];
	payload.data.uri = formatUrl(messageUri);

	// everything seems ok, we can forward message
	debug('forward message "%s"', payload.name);
	this.remoteClient.sendUTF(JSON.stringify(payload));
}
