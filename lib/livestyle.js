...
function setupPiping(tunnel) {
	// when local connection is created, update piping strategy:
	// we don’t have to gzip content but we have to effectively 
	// filter incoming messages: send only ones that can be handled
	// by remote user
	tunnel.socket.once('local', function(socket, stream) {
		// remove content gzipping
		tunnel.socket.unpipe(stream);
		stream.unpipe(tunnel.socket);
		tunnel.socket.localStream = null;

		tunnel.socket
		.pipe(new LiveStyleFilterStream())
		.pipe(socket)
		.pipe(tunnel.socket);
	});
}