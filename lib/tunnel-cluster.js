	...
	constructor(options) {
		...
		this._reconnectTimer = utils.timer(function() {
			...
			if (self._reconnectAttempts >= self.options.retryCount) {
				...
				self.destroy();
				// XXX this error event is handled by 
				// domain in Tunnel module for the actual unhandled
				// error is emitted after next attempt. Figure out how to
				// bypass this issue
				self.emit('error', new Error('Unable to connect: remote server is unreachable or session expired'));
				return;
			}
		...
		.on('tunnelError', self._reconnectTimer.restart)
		.on('connect', function() {
