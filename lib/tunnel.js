...
module.exports = class Tunnel extends EventEmitter {
	constructor(url, callback) {
		...
		self.socket
		// check if we can accept data from this socket
		.pipe(acknowledge(self.socket))
		.once('error', destroy)
		.once('available', function() {
...
/**
 * Acknowledge pipeline: ensures socket connection
 * is valid
...
function acknowledge(socket) {
	...
	return through(function(chunk, enc, next) {
		...
		var ix = buf.indexOf(CRLF);
		if (ix !== -1) {
			handled = true;
			let message = buf.slice(0, ix).toString();
			let m = message.match(/^error:\s*(.+)$/);
			if (m) {
				// connection error: can’t use this socket
				debug('socket error: %s', m[1]);
				return next(new Error(m[1].trim()));
			} else {
				this.emit('available');
				this.push(buf.slice(ix + CRLF.length));
			}
			buf = null;
		}
		next();
	}, function(next) {
		next(handled ? null : new Error('unhandled socket'));
	});
}
